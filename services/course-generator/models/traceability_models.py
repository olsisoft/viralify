...
class SourceCitationConfig(BaseModel):
    ...
    enable_vocal_citations: bool = Field(
        default=False, description="Include source citations in the voiceover narration"
    )
    citation_style: CitationStyle = Field(default=CitationStyle.NATURAL, description="Style of vocal citations")

    # Traceability panel (always available, this controls default visibility)
    show_traceability_panel: bool = Field(
        default=True, description="Show source traceability panel by default in the UI"
    )

    # What to include in traceability
    include_page_numbers: bool = Field(default=True, description="Include page numbers for PDF sources")
    include_timestamps: bool = Field(default=True, description="Include timestamps for video sources")
    include_quote_excerpts: bool = Field(default=True, description="Include short excerpts from the original source")
...
class ContentReference(BaseModel):
    ...
    # Location in source
    location: str = Field(default="", description="Location in source: 'page 45', 'timestamp 4:12', 'section 3.2'")
    ...
    # Content excerpt
    quote_excerpt: str = Field(default="", description="Short excerpt from the source that was used")

    # Relevance
    confidence: float = Field(
        default=0.8, ge=0.0, le=1.0, description="How confident we are this source was used (0-1)"
    )
    relevance_score: float = Field(
        default=0.8, ge=0.0, le=1.0, description="How relevant this source is to the content"
    )
...
class SlideTraceability(BaseModel):
    ...
    title_references: List[ContentReference] = Field(default_factory=list, description="Sources for the slide title")
    content_references: List[ContentReference] = Field(
        default_factory=list, description="Sources for the main content/bullet points"
    )
    voiceover_references: List[ContentReference] = Field(
        default_factory=list, description="Sources for the voiceover narration"
    )
    code_references: List[ContentReference] = Field(default_factory=list, description="Sources for code examples")
    diagram_references: List[ContentReference] = Field(default_factory=list, description="Sources for diagrams/visuals")

    # Aggregated metrics
    primary_source_id: Optional[str] = Field(None, description="ID of the main source used for this slide")
    source_coverage: float = Field(
        default=0.0, ge=0.0, le=1.0, description="Percentage of slide content backed by sources"
    )
...
class ConceptTraceability(BaseModel):
    ...
    lecture_ids: List[str] = Field(default_factory=list, description="IDs of lectures where this concept appears")
    slide_indices: List[int] = Field(default_factory=list, description="Slide indices where this concept is mentioned")

    # Source references by type
    definition_sources: List[ContentReference] = Field(
        default_factory=list, description="Sources providing definitions (theory role)"
    )
    example_sources: List[ContentReference] = Field(
        default_factory=list, description="Sources providing examples (example role)"
    )
    reference_sources: List[ContentReference] = Field(
        default_factory=list, description="Sources providing official references"
    )

    # Consolidated information
    consolidated_definition: Optional[str] = Field(None, description="Synthesized definition from multiple sources")
    complexity_level: int = Field(default=3, ge=1, le=5, description="Complexity level 1-5")
...
class LectureTraceability(BaseModel):
    ...
    slides: List[SlideTraceability] = Field(default_factory=list, description="Traceability for each slide")

    # Concept-level traceability
    concepts_covered: List[str] = Field(default_factory=list, description="IDs of concepts covered in this lecture")

    # Source summary
    sources_used: List[str] = Field(default_factory=list, description="IDs of all sources used in this lecture")
    primary_sources: List[str] = Field(default_factory=list, description="IDs of primary sources (most heavily used)")

    # Metrics
    overall_source_coverage: float = Field(
        default=0.0, ge=0.0, le=1.0, description="Percentage of lecture content backed by sources"
    )
...
class CourseTraceability(BaseModel):
    ...
    citation_config: SourceCitationConfig = Field(
        default_factory=SourceCitationConfig, description="Citation configuration used for this course"
    )

    # Lecture traceability
    lectures: List[LectureTraceability] = Field(default_factory=list, description="Traceability for each lecture")

    # Concept index (for quick lookup)
    concepts: List[ConceptTraceability] = Field(default_factory=list, description="All concepts covered in the course")

    # Source summary
    all_sources_used: List[str] = Field(default_factory=list, description="IDs of all sources used")
    source_usage_stats: Dict[str, Dict[str, Any]] = Field(
        default_factory=dict, description="Usage statistics per source: {source_id: {usage_count, lectures, slides}}"
    )

    # Overall metrics
    overall_source_coverage: float = Field(default=0.0, description="Percentage of course content backed by sources")
...
class TraceabilityResponse(BaseModel):
    ...
    sources_summary: List[Dict[str, Any]] = Field(
        default_factory=list, description="Summary of each source with usage stats"
    )
...
class SlideTraceabilityResponse(BaseModel):
    ...
    source_details: List[Dict[str, Any]] = Field(default_factory=list, description="Full details of sources referenced")
