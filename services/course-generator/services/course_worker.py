...
class CourseWorker:
    ...
    def __init__(self, queue_service: CourseQueueService = None, max_concurrent_jobs: int = 1):
    ...
    async def _update_job_status(
        ...
        try:
            ...
            if outline:
                ...
                print(
                    f"[WORKER] Redis update: status={job_data['status']}, outline={len(outline_json)} bytes", flush=True
                )

            await redis.hset(
                f"course_job:{job_id}",
                mapping={k: str(v) if not isinstance(v, str) else v for k, v in job_data.items()},
            )
    ...
    async def process_job(self, queued_job: QueuedCourseJob) -> None:
        ...
        try:
            ...
            await self._update_job_status(job_id, CourseJobStatus.GENERATING_OUTLINE, progress=5)
            ...
            structure = CourseStructureConfig(
                number_of_sections=queued_job.num_sections, lectures_per_section=queued_job.lectures_per_section
            )
            ...
            internal_job = CourseJob(job_id=job_id, request=generate_request)
            ...
            await self._update_job_status(
                job_id, CourseJobStatus.GENERATING_LECTURES, progress=15, outline=outline_dict
            )
            ...
            async def progress_callback(completed: int, total: int, current_title: str = None):
                ...
                await self._update_job_status(job_id, CourseJobStatus.GENERATING_LECTURES, progress=progress)
            ...
            if quiz_enabled:
                print(
                    f"[WORKER] Generating quizzes for {job_id} (frequency: {quiz_config.frequency.value})...",
                    flush=True,
                )
                ...
                try:
                    ...
                    if quizzes.get("final_quiz"):
                        ...
                        print("[WORKER] Final course quiz generated", flush=True)
                    ...
                    await self._update_job_status(
                        job_id, CourseJobStatus.GENERATING_LECTURES, progress=88, outline=outline_dict
                    )
            ...
            await self._update_job_status(job_id, CourseJobStatus.CREATING_PACKAGE, progress=90)
            ...
            await self._update_job_status(job_id, CourseJobStatus.FAILED, error=str(e))
    ...
    async def start(self) -> None:
        """Start the worker"""
        self._running = True
        print("[WORKER] Starting course worker...", flush=True)
        ...
        # Start consuming
        await self.queue_service.consume(callback=self.process_job, max_concurrent=self.max_concurrent_jobs)

    def stop(self) -> None:
        """Stop the worker gracefully"""
        print("[WORKER] Stopping worker...", flush=True)
        self._running = False
