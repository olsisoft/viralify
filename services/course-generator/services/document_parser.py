...
from services.semantic_chunker import SemanticChunk, get_semantic_chunker
            ...
            for page_num, page in enumerate(doc, 1):
                ...
                for img_index, img in enumerate(image_list):
                    try:
                        ...
                        print(
                            f"[PARSER] Error extracting image {img_index} from page {page_num}: {img_error}", flush=True
                        )
                        continue
...
class WebContentParser:
    ...
    async def parse_url(self, url: str) -> Tuple[str, dict]:
        ...
        try:
            ...
            if e.response.status_code == 404:
                ...
                raise ValueError("Accès refusé (403). Cette page bloque les robots.")
            ...
            raise ValueError("Timeout: La page met trop de temps à répondre")
        except Exception as e:
    ...
    async def parse_youtube(self, url: str) -> Tuple[str, dict]:
        ...
        try:
            print("[PARSER] Trying yt-dlp method...", flush=True)
        ...
        try:
            print("[PARSER] Trying youtube_transcript_api method...", flush=True)
        ...
        if any("unavailable" in err.lower() or "private" in err.lower() for err in errors):
            ...
            raise ValueError(
                "Impossible d'extraire la transcription YouTube. Vérifiez que la vidéo est publique et a des sous-titres."
            )
    ...
    async def _parse_youtube_with_api(self, video_id: str) -> Tuple[str, dict]:
        ...
        try:
            ...
            metadata["duration_seconds"] = (
                transcript_data[-1]["start"] + transcript_data[-1]["duration"] if transcript_data else 0
            )
            metadata["word_count"] = len(text.split())
    ...
    async def _parse_youtube_with_ytdlp(self, url: str, video_id: str) -> Tuple[str, dict]:
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            info_cmd = ["yt-dlp", "--dump-json", "--skip-download", "--no-warnings", "--quiet", url]

            try:
                # Run in executor to not block
                loop = asyncio.get_event_loop()
                result = await loop.run_in_executor(
                    None, lambda: subprocess.run(info_cmd, capture_output=True, text=True, timeout=45)
                )
                ...
                if headings:
                    ...
                    print("[PARSER] No chapters found in YouTube video", flush=True)
                ...
                print(
                    f"[PARSER] yt-dlp found subtitles: {list(subtitles.keys())}, auto: {list(auto_captions.keys())[:5]}",
                    flush=True,
                )
                ...
                result = await loop.run_in_executor(
                    None, lambda: subprocess.run(sub_cmd, capture_output=True, text=True, timeout=90)
                )
                ...
                if not sub_files:
                    ...
                    raise ValueError("Téléchargement des sous-titres échoué")
