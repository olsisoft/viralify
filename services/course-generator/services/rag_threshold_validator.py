...
class RAGThresholdValidator:
    ...
    def validate(
        ...
        Args:
            ...
            document_ids: List of document IDs (for error messages)
        ...
        if not rag_context or not rag_context.strip():
            if document_ids and len(document_ids) > 0:
                ...
                return RAGThresholdResult(
                    ...
                    is_sufficient=False,
                    error_message=(
                        f"No content could be extracted from the {len(document_ids)} "
                        "provided document(s). Please check that the documents contain "
                        "readable text content related to the topic."
                    ),
                )
            else:
    ...
    def validate_for_generation(
        ...
        """
        Validate RAG context for course/presentation generation.

        Provides more detailed error messages including the topic.

        Args:
            rag_context: The RAG context string
            topic: The generation topic (for error messages)
            document_ids: List of document IDs

        Returns:
            RAGThresholdResult with mode and messages
        """
        result = self.validate(rag_context, document_ids)
...
def validate_rag_threshold(
    ...
    Args:
        ...
        topic: Optional topic for enhanced error messages
    ...
    if topic:
        return validator.validate_for_generation(rag_context, topic, document_ids)

    return validator.validate(rag_context, document_ids)
