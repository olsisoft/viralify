...
class TestCourseWorkerInit:
    ...
    def test_custom_max_concurrent_jobs(self, mock_queue_service):
        ...
        with patch("course_worker.CourseCompositor"):
            with patch("course_worker.CoursePlanner"):
                worker = CourseWorker(queue_service=mock_queue_service, max_concurrent_jobs=5)
        ...
        with patch("course_worker.CourseCompositor"):
            with patch("course_worker.CoursePlanner"):
                ...
                await worker._update_job_status(
                    job_id="test-123", status=CourseJobStatus.GENERATING_OUTLINE, progress=25
                )
    ...
    async def test_update_job_status_with_error(self, mock_queue_service, mock_redis):
        ...
        with patch("course_worker.CourseCompositor"):
            with patch("course_worker.CoursePlanner"):
                ...
                await worker._update_job_status(
                    job_id="test-123", status=CourseJobStatus.FAILED, error="Something went wrong"
                )
    ...
    async def test_update_job_status_with_output_urls(self, mock_queue_service, mock_redis):
        ...
        with patch("course_worker.CourseCompositor"):
            with patch("course_worker.CoursePlanner"):
                ...
                output_urls = {"videos": ["http://example.com/video1.mp4"], "zip": "http://example.com/course.zip"}

                await worker._update_job_status(
                    job_id="test-123", status=CourseJobStatus.COMPLETED, progress=100, output_urls=output_urls
                )
    ...
    async def test_update_job_status_with_outline(self, mock_queue_service, mock_redis):
        ...
        with patch("course_worker.CourseCompositor"):
            with patch("course_worker.CoursePlanner"):
                ...
                await worker._update_job_status(
                    job_id="test-123", status=CourseJobStatus.GENERATING_LECTURES, progress=15, outline=outline
                )
        ...
        with patch("course_worker.CourseCompositor"):
            with patch("course_worker.CoursePlanner"):
                ...
                with pytest.raises(Exception) as excinfo:
                    await worker._update_job_status(
                        job_id="test-123", status=CourseJobStatus.GENERATING_OUTLINE, progress=5
                    )
...
class TestJobProcessing:
    ...
    @pytest.mark.asyncio
    async def test_process_job_success(self, mock_queue_service, mock_redis, sample_queued_job, sample_outline):
    ...
    async def test_process_job_tracks_current_job(
        self, mock_queue_service, mock_redis, sample_queued_job, sample_outline
    ):
    ...
    @pytest.mark.asyncio
    async def test_process_job_failure(self, mock_queue_service, mock_redis, sample_queued_job):
        """Test job processing failure handling"""
        with patch("course_worker.CourseCompositor"):
            with patch("course_worker.CoursePlanner") as MockPlanner:
                mock_planner = MockPlanner.return_value
                mock_planner.generate_outline = AsyncMock(side_effect=Exception("LLM API error"))
    ...
    @pytest.mark.asyncio
    async def test_process_job_with_quiz_config(self, mock_queue_service, mock_redis, sample_outline):
    ...
    @pytest.mark.asyncio
    async def test_process_job_with_document_ids(self, mock_queue_service, mock_redis, sample_outline):
        ...
        with patch("course_worker.CourseCompositor") as MockCompositor:
            with patch("course_worker.CoursePlanner") as MockPlanner:
                with patch("course_worker.RAGService") as MockRAG:
                    ...
                    mock_rag.get_context_for_course_generation = AsyncMock(return_value="RAG context content...")
...
class TestWorkerLifecycle:
    ...
    def test_get_current_jobs(self, mock_queue_service):
        ...
        with patch("course_worker.CourseCompositor"):
            with patch("course_worker.CoursePlanner"):
                worker = CourseWorker(queue_service=mock_queue_service)
                worker._current_jobs = {"job-1": "data1", "job-2": "data2"}
