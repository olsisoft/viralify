...
from models.data_models import AcousticAnalysisResult, Anomaly, AnomalyType, SeverityLevel, TimeRange
...
class AcousticAnalyzer:
    ...
    def _merge_adjacent_anomalies(self, anomalies: List[Anomaly], gap_threshold_ms: int = 100) -> List[Anomaly]:
        """Fusionne les anomalies adjacentes du même type"""
        if not anomalies:
            return []

        # Trier par type puis par temps de début
        sorted_anomalies = sorted(anomalies, key=lambda a: (a.anomaly_type.value, a.time_range.start_ms))
        ...
        for anomaly in sorted_anomalies[1:]:
            if (
                anomaly.anomaly_type == current.anomaly_type
                and anomaly.time_range.start_ms - current.time_range.end_ms < gap_threshold_ms
            ):
                # Fusionner
                current = Anomaly(
                    anomaly_type=current.anomaly_type,
                    severity=max(current.severity, anomaly.severity, key=lambda s: list(SeverityLevel).index(s)),
                    time_range=TimeRange(current.time_range.start_ms, anomaly.time_range.end_ms),
    ...
    def _compute_acoustic_score(self, anomalies: List[Anomaly], duration_ms: int) -> float:
        """
