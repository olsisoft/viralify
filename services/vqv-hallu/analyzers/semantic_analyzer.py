...
from config.settings import ContentTypeConfig
...
class SemanticAnalyzer:
    ...
    def analyze(self, source_text: str, transcription: TranscriptionResult) -> SemanticAnalysisResult:
        """
        ...
        overall_similarity = self._compute_embedding_similarity(source_text, transcript_text)

        # 2. Alignement mot-à-mot
        word_alignments = self._align_words(source_text, transcript_text, transcription.word_timestamps)

        # 3. Détection des frontières d'hallucination
        hallucination_result = self._detect_hallucination_boundaries(source_text, transcript_text, transcription)
        anomalies.extend(hallucination_result["anomalies"])

        # 4. Calcul de la dérive sémantique
        semantic_drift = self._compute_semantic_drift(source_text, transcript_text)

        # 5. Analyse de couverture du contenu
        coverage_result = self._analyze_content_coverage(source_text, transcript_text, word_alignments)
        anomalies.extend(coverage_result["anomalies"])

        # 6. Détection de contenu extra
        extra_result = self._detect_extra_content(source_text, transcript_text, word_alignments)
    ...
    async def analyze_async(
        self, source_text: str, transcription: TranscriptionResult, user_id: Optional[str] = None
    ) -> SemanticAnalysisResult:
        """
        ...
        weave_graph_boost = await self._compute_weave_graph_boost(source_text, transcript_text, user_id)
        ...
        score = self._compute_semantic_score_with_boost(
            anomalies, boosted_similarity, semantic_drift, coverage_result["content_coverage"], weave_graph_boost
        )
    ...
    async def _compute_weave_graph_boost(
        self, source_text: str, transcript_text: str, user_id: Optional[str]
        ...
        if not client or not user_id:
            ...
            return WeaveGraphBoostResult(
                boost=0.0, concept_integrity_score=0.0, matched_concepts=[], missing_concepts=[], phonetic_matches=[]
            )

        try:
            # Vérifier l'intégrité des concepts
            result = await client.check_concept_integrity(
                source_text=source_text, transcription_text=transcript_text, user_id=user_id
            )
    ...
    def _compute_semantic_score_with_boost(
        ...
        for anomaly in anomalies:
            ...
            if anomaly.anomaly_type == AnomalyType.HALLUCINATION and "phonétique" in anomaly.description.lower():
    ...
    def _compute_embedding_similarity(self, text1: str, text2: str) -> float:
        ...
        embeddings = self.embedding_model.encode([text1, text2], convert_to_tensor=True, show_progress_bar=False)

        # Similarité cosinus
        similarity = float(
            cosine_similarity(embeddings[0].cpu().numpy().reshape(1, -1), embeddings[1].cpu().numpy().reshape(1, -1))[
                0, 0
            ]
        )
    ...
    def _align_words(self, source_text: str, transcript_text: str, word_timestamps: List[Dict]) -> List[WordAlignment]:
        """
    ...
    def _detect_hallucination_boundaries(
        self, source_text: str, transcript_text: str, transcription: TranscriptionResult
    ) -> dict:
        """
        ...
        sim_matrix = cosine_similarity(source_embeddings.cpu().numpy(), transcript_embeddings.cpu().numpy())

        # Pour chaque phrase de la transcription, trouver la meilleure correspondance
        word_timestamps = transcription.word_timestamps
        char_to_timestamp = self._build_char_timestamp_map(transcript_text, word_timestamps)
        ...
        for i, trans_sent in enumerate(transcript_sentences):
            ...
            time_range = self._get_timestamp_range(char_to_timestamp, sent_start, sent_end)
        ...
        for i in range(1, len(transcript_sentences)):
            ...
            if prev_sim - curr_sim > 0.3:
                ...
                time_range = self._get_timestamp_range(char_to_timestamp, sent_start, sent_start + 10)
    ...
    def _compute_semantic_drift(self, source_text: str, transcript_text: str) -> float:
        """
    ...
    def _analyze_content_coverage(
        self, source_text: str, transcript_text: str, alignments: List[WordAlignment]
    ) -> dict:
        """
        ...
        missing_words = [
            a.source_word for a in alignments if not a.is_match and a.source_word and not a.transcribed_word
        ]

        if content_coverage < 0.8:
            severity = SeverityLevel.HIGH if content_coverage < 0.5 else SeverityLevel.MEDIUM
    ...
    def _detect_extra_content(self, source_text: str, transcript_text: str, alignments: List[WordAlignment]) -> dict:
        """
        ...
        extra_words = [
            a.transcribed_word for a in alignments if not a.is_match and a.transcribed_word and not a.source_word
        ]
    ...
    def _build_char_timestamp_map(self, text: str, word_timestamps: List[Dict]) -> Dict[int, int]:
        """
    ...
    def _get_timestamp_range(self, char_map: Dict[int, int], start_char: int, end_char: int) -> TimeRange:
    ...
    def _compute_semantic_score(
        self, anomalies: List[Anomaly], overall_similarity: float, semantic_drift: float, content_coverage: float
    ) -> float:
        """
