...
from models.data_models import VQVAnalysisResult, VQVInputMessage, VQVOutputMessage
...
class VQVHalluPipeline:
    ...
    def __init__(self, global_config: VQVHalluConfig, use_adaptive_fusion: bool = True):
        """
    ...
    def _get_semantic_analyzer(self, config: ContentTypeConfig) -> SemanticAnalyzer:
        ...
        if key not in self._semantic_analyzers:
            self._semantic_analyzers[key] = SemanticAnalyzer(config, embedding_model=self.global_config.embedding_model)
    ...
    def analyze(
        self, audio_path: str, source_text: str, audio_id: str, content_type: str = "mixed", language: str = "fr"
    ) -> VQVAnalysisResult:
        """
        ...
        try:
            ...
            semantic_result = semantic_analyzer.analyze(source_text, linguistic_result.transcription)
...
def analyze_voiceover(
    ...
    return pipeline.analyze(
        audio_path=audio_path, source_text=source_text, audio_id=audio_id, content_type=content_type, language=language
    )
