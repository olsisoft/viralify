...
class ScoreFusionEngine:
    ...
    def fuse(
        ...
        base_score = self._compute_weighted_score(acoustic_result.score, linguistic_result.score, semantic_result.score)

        # 2. Détection de patterns cross-layer
        cross_layer_penalty = self._detect_cross_layer_patterns(
            acoustic_result.anomalies, linguistic_result.anomalies, semantic_result.anomalies
        )

        # 3. Calibration de confiance
        confidence_adjustment = self._calibrate_confidence(acoustic_result, linguistic_result, semantic_result)

        # 4. Score final
        final_score = max(0.0, min(100.0, base_score - cross_layer_penalty + confidence_adjustment))

        # 5. Générer le verdict
        is_acceptable = final_score >= self.min_acceptable_score
        primary_issues = self._identify_primary_issues(acoustic_result, linguistic_result, semantic_result)
        recommended_action = self._determine_action(final_score, primary_issues)
    ...
    def _compute_weighted_score(self, acoustic_score: float, linguistic_score: float, semantic_score: float) -> float:
        """
    ...
    def _detect_cross_layer_patterns(
        self, acoustic_anomalies: List[Anomaly], linguistic_anomalies: List[Anomaly], semantic_anomalies: List[Anomaly]
    ) -> float:
        """
        ...
        triple_penalty = self._detect_triple_pattern(acoustic_anomalies, linguistic_anomalies, semantic_anomalies)
        total_penalty += triple_penalty

        return total_penalty

    def _check_temporal_overlap(self, anomalies1: List[Anomaly], anomalies2: List[Anomaly]) -> float:
        """
        ...
        for a_anom in acoustic_anomalies:
            for l_anom in linguistic_anomalies:
                ...
                for s_anom in semantic_anomalies:
                    if a_anom.time_range.overlaps(s_anom.time_range) and l_anom.time_range.overlaps(s_anom.time_range):
    ...
    def _calibrate_confidence(
        ...
        """
        scores = [acoustic_result.score, linguistic_result.score, semantic_result.score]
    ...
    def _identify_primary_issues(
        ...
        all_anomalies = acoustic_result.anomalies + linguistic_result.anomalies + semantic_result.anomalies

        # Trier par sévérité puis par confiance
        sorted_anomalies = sorted(all_anomalies, key=lambda a: (-list(SeverityLevel).index(a.severity), -a.confidence))
        ...
        if linguistic_result.mean_word_confidence < 0.6:
            issues.append(f"Confiance ASR globale faible ({linguistic_result.mean_word_confidence:.0%})")

        if semantic_result.overall_similarity < 0.7:
            issues.append(f"Alignement sémantique faible ({semantic_result.overall_similarity:.0%})")

        if acoustic_result.distortion_score > 0.15:
            issues.append(f"Distorsion audio détectée (THD={acoustic_result.distortion_score:.1%})")
...
class AdaptiveScoreFusion(ScoreFusionEngine):
    ...
    def _compute_weighted_score(self, acoustic_score: float, linguistic_score: float, semantic_score: float) -> float:
        ...
        # Ajuster les poids
        adjusted_weights = {k: self.base_weights[k] * reliabilities[k] for k in self.base_weights}
