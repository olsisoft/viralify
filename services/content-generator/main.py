...
class TikTokAIAgents:
    ...
    def __init__(self):
        self.openai_llm = ChatOpenAI(model=_get_model_name("fast"), temperature=0.7, openai_api_key=OPENAI_API_KEY)
        ...
        if anthropic_key_valid:
            self.anthropic_llm = ChatAnthropic(
                model="claude-3-opus-20240229", temperature=0.8, anthropic_api_key=ANTHROPIC_API_KEY
            )
        else:
    ...
    def _initialize_agents(self):
        ...
        trend_prompt = ChatPromptTemplate.from_messages(
            [
                (
                    "system",
                    """You are TrendScout, an expert AI agent specialized in analyzing TikTok trends.
Your role is to:
...
Use your tools to gather real-time trend data before making recommendations.""",
        ...
        self.agents["TrendScout"] = AgentExecutor(
            agent=trend_agent, tools=trend_tools, verbose=True, max_iterations=5, handle_parsing_errors=True
        )
        ...
        script_prompt = ChatPromptTemplate.from_messages(
            [
                (
                    "system",
                    """You are ScriptGenius, a creative AI agent specialized in writing TikTok video scripts.
...
Consider the target audience, current trends, and platform best practices.""",
        ...
        self.agents["ScriptGenius"] = AgentExecutor(
            agent=script_agent, tools=script_tools, verbose=True, max_iterations=5, handle_parsing_errors=True
        )
        ...
        optimizer_prompt = ChatPromptTemplate.from_messages(
            [
                (
                    "system",
                    """You are ContentOptimizer, an AI agent focused on maximizing TikTok content performance.
Your capabilities:
...
Base recommendations on platform algorithms, audience behavior, and proven strategies.""",
        ...
        self.agents["ContentOptimizer"] = AgentExecutor(
            agent=optimizer_agent, tools=optimizer_tools, verbose=True, max_iterations=5, handle_parsing_errors=True
        )
        ...
        strategy_prompt = ChatPromptTemplate.from_messages(
            [
                (
                    "system",
                    """You are StrategyAdvisor, a strategic AI agent for TikTok growth.
...
Provide strategic, actionable advice that balances short-term wins with long-term growth.""",
        ...
        self.agents["StrategyAdvisor"] = AgentExecutor(
            agent=strategy_agent, tools=strategy_tools, verbose=True, max_iterations=5, handle_parsing_errors=True
        )
    ...
    async def _predict_content_score(self, content_data: str) -> str:
        ...
        return json.dumps(
            {
                "predicted_score": 7.5,
                "factors": {"hook_strength": 8, "content_value": 7, "trend_alignment": 8, "cta_clarity": 7},
            }
        )
    ...
    async def _generate_content_calendar(self, days: int) -> str:
        ...
        for i in range(days):
            calendar.append(
                {"day": i + 1, "content_type": content_types[i % len(content_types)], "posting_time": "19:00"}
            )
    ...
    async def _project_growth(self, strategy: str) -> str:
        ...
        return json.dumps(
            {
                "30_day_projection": {"follower_growth": "+15-25%", "avg_views": "+30%", "engagement_rate": "+10%"},
    ...
    async def generate_script(self, request: GenerateScriptRequest) -> ScriptResponse:
        ...
        if request.system_prompt and request.format == "structured":
            ...
            try:
                ...
                result = await asyncio.to_thread(self.anthropic_llm.invoke, messages)
        ...
        if not scenes:
            ...
            trend_result = await asyncio.to_thread(
                self.agents["TrendScout"].invoke, {"input": trend_input, "chat_history": []}
            )
...
Make visuals specific and descriptive for AI image generation."""
            ...
            script_result = await asyncio.to_thread(
                self.agents["ScriptGenius"].invoke, {"input": script_input, "chat_history": []}
            )
            ...
            if json_match:
                ...
                for scene in scenes_json:
                    ...
                    if isinstance(visual_raw, dict):
                        visual = (
                            visual_raw.get("image", "")
                            or visual_raw.get("description", "")
                            or visual_raw.get("visual", "")
                            or str(visual_raw)
                        )
                    else:
                    ...
                    if isinstance(audio_raw, dict):
                        audio = (
                            audio_raw.get("text", "")
                            or audio_raw.get("audio", "")
                            or audio_raw.get("voiceover", "")
                            or str(audio_raw)
                        )
                    else:
...
- Make it scroll-stopping"""
        ...
        result = await asyncio.to_thread(
            self.agents["ContentOptimizer"].invoke, {"input": input_text, "chat_history": []}
        )
    ...
    async def chat_with_agent(self, request: AIAgentChatRequest) -> ChatResponse:
        ...
        result = await asyncio.to_thread(agent.invoke, {"input": full_input, "chat_history": []})
...
async def generate_hashtags(request: GenerateHashtagsRequest):
    ...
    try:
        ...
        result = await asyncio.to_thread(
            ai_agents.agents["ContentOptimizer"].invoke, {"input": input_text, "chat_history": []}
        )
...
async def create_content_strategy(request: ContentStrategyRequest):
    ...
    try:
        ...
        result = await asyncio.to_thread(
            ai_agents.agents["StrategyAdvisor"].invoke, {"input": input_text, "chat_history": []}
        )
        ...
        return StrategyResponse(
            ...
            growth_projections={"followers_30d": "+500-1000", "avg_views": "+50%", "engagement_rate": "5-8%"},
