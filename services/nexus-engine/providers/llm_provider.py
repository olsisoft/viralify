...
def try_parse_json(content: str) -> Dict[str, Any]:
    ...
    # Strategy 6: Detect raw code response and wrap in minimal JSON
    # This handles cases where LLM returns code directly instead of JSON
    stripped = content.strip()
    if stripped.startswith(("def ", "class ", "import ", "from ", "async ", "#", "//")) or re.match(
        r"^(function|const|let|var|public|private)\s", stripped
    ):
        logger.warning("LLM returned raw code instead of JSON, wrapping in minimal structure")
        # Detect language from content
        language = "python"
        if re.match(r"^(function|const|let|var|export|import\s+\{)", stripped):
            language = "javascript"
        elif re.match(r"^(public|private|class\s+\w+\s*\{)", stripped) and "void" in stripped:
            language = "java"
...
class BaseLLMProvider(ABC):
    ...
    def generate_json(self, messages: List[LLMMessage], schema_hint: str = "", **kwargs) -> Dict[str, Any]:
        """
        ...
        for json_attempt in range(max_json_retries):
            ...
            try:
                ...
                if "json_validate_failed" in error_str:
                    logger.warning("JSON mode failed with json_validate_failed")
                    ...
                    try:
                        ...
                        if match:
                            ...
                            logger.info("Extracted failed_generation, attempting to parse...")
                            try:
                    ...
                    logger.info("Retrying without JSON mode...")
            ...
            if not response.content or not response.content.strip():
                ...
                logger.warning(
                    f"Empty LLM response (attempt {json_attempt + 1}/{max_json_retries}), retrying in {backoff_time}s..."
                )
                if json_attempt < max_json_retries - 1:
                    ...
                    raise ValueError(
                        f"LLM returned empty response after {max_json_retries} attempts. This may indicate rate limiting or API issues."
                    )
            ...
            try:
                ...
                # Handle any code block format: ```json, ```python, ```javascript, etc.
                if content.startswith("```"):
                    # Find the end of the language identifier (first newline after ```)
                ...
                if not content:
                    ...
                    logger.warning(
                        f"Response empty after cleaning (attempt {json_attempt + 1}/{max_json_retries}), retrying in {backoff_time}s..."
                    )
                    if json_attempt < max_json_retries - 1:
                        time.sleep(backoff_time)
                        continue
