...
app = FastAPI(
    title="Notification Service", description="Real-time notifications, alerts, and communication", version="1.0.0"
)
...
NOTIFICATION_TEMPLATES = {
    ...
    NotificationType.AI_SUGGESTION: {"title": "💡 AI Content Suggestion", "message": "{suggestion}"},
    ...
    async def create_notification(
        ...
        async with async_session() as session:
            notification = Notification(
                user_id=user_id, type=notification_type.value, title=final_title, message=final_message, data=data or {}
            )
            ...
            await manager.send_to_user(str(user_id), {"type": "notification", "data": response.dict()})

            # Publish to Redis for other services
            if redis_client:
                await redis_client.publish(f"notifications:{user_id}", json.dumps(response.dict(), default=str))
...
async def trigger_post_failed(user_id: UUID, post_title: str, error: str):
    ...
    return await notification_service.create_notification(
        user_id=user_id, notification_type=NotificationType.POST_FAILED, data={"title": post_title, "error": error}
    )
...
async def trigger_engagement_milestone(user_id: UUID, post_title: str, milestone: int, metric: str):
