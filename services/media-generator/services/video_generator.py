...
from .asset_fetcher import AssetFetcherService, MediaType
...
class AIVideoGenerator:
    ...
    def _get_did_provider(self):
        ...
        if self._did_provider is None and self.did_api_key:
            ...
            self._did_provider = DIDProvider(api_key=self.did_api_key, output_dir="/tmp/viralify/lipsync")
        return self._did_provider

    async def generate_video(
        self, request: VideoGenerationRequest, user_id: str = "demo-user", progress_callback: Optional[Callable] = None
    ) -> VideoGenerationJob:
        """
        ...
        job = VideoGenerationJob(
            ...
            stages={
                GenerationStage.PLANNING.value: StageProgress(
                    stage=GenerationStage.PLANNING, progress=0, message="Initializing..."
                ),
                GenerationStage.FETCHING_ASSETS.value: StageProgress(stage=GenerationStage.FETCHING_ASSETS),
                GenerationStage.GENERATING_VOICEOVER.value: StageProgress(stage=GenerationStage.GENERATING_VOICEOVER),
                GenerationStage.FETCHING_MUSIC.value: StageProgress(stage=GenerationStage.FETCHING_MUSIC),
                GenerationStage.COMPOSING.value: StageProgress(stage=GenerationStage.COMPOSING),
            },
        )
    ...
    async def _run_generation(
        self, job: VideoGenerationJob, request: VideoGenerationRequest, progress_callback: Optional[Callable] = None
    ):
        ...
        try:
            ...
            project = await self.planner.plan_video(
                prompt=request.prompt, duration=request.duration, style=request.style, format=request.format
            )
            ...
            voiceover_url, word_timestamps = await self._generate_voiceover(
                project.voiceover_text, request.voice_id, request.voice_provider
            )
            ...
            if request.enable_lipsync and voiceover_url:
                ...
                await self._update_stage(
                    job, GenerationStage.GENERATING_VOICEOVER, 60, "Generating lip-sync animation..."
                )
            ...
            if request.include_music:
                ...
                music_track = await self.music_service.get_best_track_for_mood(
                    target_mood, min_duration=request.duration
                )
                if music_track:
            ...
            def composition_progress(percent, message):
                asyncio.create_task(self._update_stage(job, GenerationStage.COMPOSING, percent, message))

            result = await self.compositor.compose_video(composition_request, progress_callback=composition_progress)
    ...
    async def generate_video_from_project(
        self, project: VideoProject, request: VideoGenerationRequest, user_id: str = "demo-user"
    ) -> VideoGenerationJob:
        """
        ...
        job = VideoGenerationJob(
            ...
            stages={
                GenerationStage.PLANNING.value: StageProgress(
                    stage=GenerationStage.PLANNING, progress=100, message="Script provided - skipped"
                ),
                GenerationStage.FETCHING_ASSETS.value: StageProgress(
                    stage=GenerationStage.FETCHING_ASSETS, progress=0, message="Starting..."
                ),
                GenerationStage.GENERATING_VOICEOVER.value: StageProgress(stage=GenerationStage.GENERATING_VOICEOVER),
                GenerationStage.FETCHING_MUSIC.value: StageProgress(stage=GenerationStage.FETCHING_MUSIC),
                GenerationStage.COMPOSING.value: StageProgress(stage=GenerationStage.COMPOSING),
    ...
    async def _generate_from_existing_project(
        self, job: VideoGenerationJob, project: VideoProject, request: VideoGenerationRequest
    ):
        ...
        try:
            ...
            print(
                f"Stage 2: Generating voiceover (text length: {len(project.voiceover_text) if project.voiceover_text else 0})...",
                flush=True,
            )
            voiceover_url = None
            word_timestamps = []
            if project.voiceover_text:
                ...
                voiceover_url, word_timestamps = await self._generate_voiceover(
                    text=project.voiceover_text, voice_id=request.voice_id, provider=request.voice_provider
                )
                await self._update_stage(job, GenerationStage.GENERATING_VOICEOVER, 100, "Voiceover ready!")
            ...
            if request.include_music:
                ...
                music_track = await self.music_service.get_best_track_for_mood(
                    request.music_style or project.music_style or "cinematic", min_duration=request.duration
                )
                if music_track:
                    music_url = music_track.url
                    project.music_url = music_url
            ...
            print("[COMPOSITION] Starting composition...", flush=True)
            print(
                f"[COMPOSITION] {scenes_with_media}/{len(project.scenes)} scenes have media ({scenes_without_media} missing)",
                flush=True,
            )

            # Log all scene details before composition
            print("[COMPOSITION] Scene details:", flush=True)
            for idx, s in enumerate(project.scenes):
                print(
                    f"[COMPOSITION]   Scene {idx + 1}: type={s.scene_type}, duration={s.duration}s, media={s.media_url[:60] if s.media_url else 'None'}...",
                    flush=True,
                )
    ...
    async def _fetch_scene_assets(
            ...
            try:
                ...
                if scene.scene_type == SceneType.AI_IMAGE:
                    ...
                    asset = await self.asset_fetcher.generate_ai_image(
                        prompt=scene.description, style=project.style, aspect_ratio=aspect_ratio
                    )
                ...
                await self._update_stage(
                    job, GenerationStage.FETCHING_ASSETS, progress, f"Fetched {index + 1}/{total_scenes} assets"
                )
        ...
        tasks = [fetch_scene_asset(scene, i) for i, scene in enumerate(project.scenes)]
        await asyncio.gather(*tasks)

    async def _generate_voiceover(
        self, text: str, voice_id: str, provider: str
            ...
            if provider == "elevenlabs" and self.elevenlabs_key:
                ...
                async with httpx.AsyncClient() as client:
                    ...
                    if response.status_code == 200:
                        ...
                        if audio_base64:
                            ...
                            with tempfile.NamedTemporaryFile(
                                suffix=".mp3", delete=False, dir="/tmp/viralify/audio"
                            ) as f:
                                    ...
                                    if char == " " or i == len(characters) - 1:
                                        ...
                                        if current_word.strip():
                                            ...
                                            word_timestamps.append(
                                                WordTimestamp(word=current_word.strip(), start=word_start, end=word_end)
                                            )
                    ...
                    if response.status_code == 200:
                        ...
                        with tempfile.NamedTemporaryFile(suffix=".mp3", delete=False, dir="/tmp/viralify/audio") as f:
    ...
    async def _update_stage(self, job: VideoGenerationJob, stage: GenerationStage, progress: int, message: str):
    ...
    async def update_scene(self, job_id: str, scene_id: str, updates: Dict[str, Any]) -> Optional[Scene]:
    ...
    async def add_scene(
        self, job_id: str, scene_data: Dict[str, Any], after_scene_id: Optional[str] = None
    ) -> Optional[Scene]:
    ...
    async def _process_lipsync_scenes(
        ...
        if avatar_id:
            ...
            avatar_service = AvatarService(
                did_api_key=os.getenv("DID_API_KEY"), heygen_api_key=os.getenv("HEYGEN_API_KEY")
            )
            ...
            if avatar:
                ...
                if default_avatar and default_avatar.did_presenter_id:
                    ...
                    print("[LIP-SYNC] ERROR: Could not find avatar or fallback", flush=True)
        ...
        print("[LIP-SYNC] Skipping early background removal (handled by avatar service)", flush=True)
        ...
        for i, scene in enumerate(project.scenes):
            ...
            is_presenter_scene = any(
                keyword in scene_desc
                for keyword in [
                    "person",
                    "presenter",
                    "speaker",
            ...
            print(
                f"[LIP-SYNC] Scene {i + 1}: is_presenter_scene={is_presenter_scene}, desc[:50]={scene_desc[:50]}",
                flush=True,
            )
            ...
            try:
                print(
                    f"[LIP-SYNC] Processing scene {i + 1}: {scene.description[:50] if scene.description else 'no desc'}...",
                    flush=True,
                )
                ...
                if did_audio_url:
                    ...
                    if use_hybrid:
                        print("[LIP-SYNC] Using HYBRID provider (Replicate → D-ID fallback)...", flush=True)
                        try:
                            ...
                            if face_swap_image:
                                print(
                                    f"[LIP-SYNC] Face swap enabled (hair_source: {face_swap_hair_source})", flush=True
                                )
                    ...
                    if (
                        use_presenter
                        and avatar
                        and hasattr(avatar, "did_clip_presenter_id")
                        and avatar.did_clip_presenter_id
                    ):
                        print("[LIP-SYNC] Using D-ID Clips API for FULL BODY movement", flush=True)
                    ...
                    print("[LIP-SYNC] Calling D-ID create_talk...", flush=True)
                ...
                # Poll for completion
                print("[LIP-SYNC] Polling for talk completion...", flush=True)
                ...
                if result and result.get("result_url"):
                    ...
                    print("[LIP-SYNC] ERROR: No result_url in D-ID response", flush=True)
                    lipsync_video_path = None

                # Update progress
                progress = 60 + int((processed_count / max(1, len(project.scenes))) * 40)
                await self._update_stage(
                    job, GenerationStage.GENERATING_VOICEOVER, progress, "Lip-sync: avatar video ready"
                )
        ...
        if processed_count > 0:
            print("[LIP-SYNC] COMPLETE: Lip-sync video generated", flush=True)
