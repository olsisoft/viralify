...
class Settings:
    DATABASE_URL: str = os.getenv(
        "DATABASE_URL", "postgresql://viralify_prod:password@localhost:5432/viralify_production"
    )
...
class ComposeSlideshowRequest(BaseModel):
    ...
    ken_burns_effect: bool = Field(
        default=False, description="Enable Ken Burns zoom/pan effect on images. False for static display."
    )
    # PIP Avatar (medallion) settings
    pip_avatar_url: Optional[str] = Field(None, description="URL to avatar video for PIP overlay")
    pip_position: str = Field(
        default="bottom-right", pattern=r"^(top-left|top-right|bottom-left|bottom-right)$", description="PIP position"
    )
    ...
    async with httpx.AsyncClient() as client:
        ...
        try:
            response = await client.post(
                ...
                json={
                    "model": "gpt-4o",
                    "messages": [{"role": "user", "content": f"Generate a photorealistic image: {enhanced_prompt}"}],
                    ...
                    for content in message.get("content", []):
                        if content.get("type") == "image":
                            ...
                            if "url" in image_data:
                                ...
                                return {"url": image_path, "revised_prompt": enhanced_prompt, "provider": "gpt-4o"}
                print("GPT-4o response didn't contain image, trying images API...")
            else:
        ...
        try:
            size_map = {"1:1": "1024x1024", "9:16": "1024x1536", "16:9": "1536x1024"}
        ...
        if settings.GEMINI_API_KEY:
            try:
                ...
                gemini_response = await client.post(
                    ...
                    json={
                        "contents": [{"parts": [{"text": f"Generate a photorealistic image: {enhanced_prompt}"}]}],
                        ...
                        for part in content.get("parts", []):
                            if "inlineData" in part:
                                ...
                                if inline_data.get("mimeType", "").startswith("image/"):
                                    ...
                                    print("Gemini 2.0 Flash generated image successfully")
                                    return {
                                        "url": image_path,
                                        "revised_prompt": enhanced_prompt,
                                        "provider": "gemini-2.0-flash",
                                    }
                    print("Gemini response didn't contain image data")
                elif gemini_response.status_code == 429:
                    print("Gemini quota exceeded, falling back to DALL-E 3...")
                else:
        ...
        dalle_size_map = {"1:1": "1024x1024", "9:16": "1024x1792", "16:9": "1792x1024"}
        dalle_size = dalle_size_map.get(aspect_ratio, "1024x1792")

        response = await client.post(
            "https://api.openai.com/v1/images/generations",
            headers={"Authorization": f"Bearer {settings.OPENAI_API_KEY}", "Content-Type": "application/json"},
            json={"model": "dall-e-3", "prompt": enhanced_prompt, "n": 1, "size": dalle_size, "quality": quality},
            timeout=90.0,
        )
...
async def generate_diagram_dalle(description: str, style: str, aspect_ratio: str) -> Dict:
    ...
    if settings.DEMO_MODE:
        await asyncio.sleep(2)
        return {
            "url": "https://placehold.co/1080x1920/1e3a5f/ffffff?text=Diagram",
            "diagram_url": "https://placehold.co/1080x1920/1e3a5f/ffffff?text=Diagram",
            "provider": "gpt-image-1",
        }

    # Map aspect ratio to GPT-Image-1 sizes
    size_mapping = {"9:16": "1024x1536", "16:9": "1536x1024", "1:1": "1024x1024"}
    ...
    async with httpx.AsyncClient() as client:
        ...
        try:
            response = await client.post(
                "https://api.openai.com/v1/images/generations",
                headers={"Authorization": f"Bearer {settings.OPENAI_API_KEY}", "Content-Type": "application/json"},
                json={"model": "gpt-image-1", "prompt": diagram_prompt, "n": 1, "size": size, "quality": "high"},
                timeout=120.0,
            )
        ...
        dalle_size_mapping = {"9:16": "1024x1792", "16:9": "1792x1024", "1:1": "1024x1024"}
        dalle_size = dalle_size_mapping.get(aspect_ratio, "1024x1792")

        response = await client.post(
            "https://api.openai.com/v1/images/generations",
            headers={"Authorization": f"Bearer {settings.OPENAI_API_KEY}", "Content-Type": "application/json"},
            json={"model": "dall-e-3", "prompt": diagram_prompt, "n": 1, "size": dalle_size, "quality": "hd"},
            timeout=90.0,
        )
...
async def generate_voiceover_openai(text: str, voice_id: str, speed: float) -> Dict:
    ...
    async with httpx.AsyncClient() as client:
        response = await client.post(
            ...
            headers={"Authorization": f"Bearer {settings.OPENAI_API_KEY}", "Content-Type": "application/json"},
            json={"model": "tts-1-hd", "input": text, "voice": voice_id, "speed": speed},
            timeout=120.0,
        )
...
async def generate_voiceover_elevenlabs(text: str, voice_id: str, emotion: Optional[str], language: str = "en") -> Dict:
    ...
    async with httpx.AsyncClient() as client:
        ...
        full_url = get_public_url(f"api/v1/media/audio/{audio_id}")
        return {"url": full_url, "audio_id": audio_id, "duration_seconds": len(text) / 12, "provider": "elevenlabs"}


async def search_pexels_videos(
    query: str, duration_max: int, orientation: str, per_page: int
...
async def process_image_job(job_id: str, request: GenerateImageRequest, user_id: str):
    ...
    try:
        result = await generate_image_dalle(request.prompt, request.style, request.aspect_ratio, request.quality)
...
async def process_voiceover_job(job_id: str, request: GenerateVoiceoverRequest, user_id: str):
    ...
    try:
        ...
        if request.provider == VoiceProvider.ELEVENLABS or language != "en":
            result = await generate_voiceover_elevenlabs(request.text, request.voice_id, request.emotion, language)
        else:
            result = await generate_voiceover_openai(request.text, request.voice_id, request.speed)
...
async def process_diagram_job(job_id: str, request: GenerateDiagramRequest, user_id: str):
    ...
    try:
        result = await generate_diagram_dalle(request.description, request.style, request.aspect_ratio)
...
async def clear_cache():
    ...
    return {"status": "success", "message": f"Cleared {result['cleared']} cached videos"}
...
async def generate_thumbnail(
    request: GenerateThumbnailRequest, background_tasks: BackgroundTasks, user_id: str = "demo-user"
):
    ...
    image_request = GenerateImageRequest(prompt=prompt, style=request.style, aspect_ratio="9:16", quality="standard")
...
async def generate_diagram(
    request: GenerateDiagramRequest, background_tasks: BackgroundTasks, user_id: str = "demo-user"
):
    ...
    try:
        ...
        return {"job_id": job_id, "status": "completed", "url": result["url"], "diagram_url": result["diagram_url"]}
...
async def generate_voiceover(
    request: GenerateVoiceoverRequest, background_tasks: BackgroundTasks, user_id: str = "demo-user"
):
...
async def search_stock_videos(request: SearchStockVideoRequest):
    ...
    return await search_pexels_videos(request.query, request.duration_max, request.orientation, request.per_page)


# Video Composition
@app.post("/api/v1/media/video/compose", response_model=JobResponse)
async def compose_video(
    request: SimpleComposeVideoRequest, background_tasks: BackgroundTasks, user_id: str = "demo-user"
):
...
async def compose_slideshow(
    request: ComposeSlideshowRequest, background_tasks: BackgroundTasks, user_id: str = "demo-user"
):
    """
...
async def generate_article(
    request: GenerateArticleRequest, background_tasks: BackgroundTasks, user_id: str = "demo-user"
):
...
async def get_user_jobs(user_id: str = "demo-user", limit: int = 20):
    ...
    user_jobs = [JobResponse(**job) for job in jobs_db.values() if job.get("user_id") == user_id]
...
async def get_user_assets(user_id: str = "demo-user", asset_type: Optional[str] = None, limit: int = 50):
...
class GenerateVideoFromPromptRequest(BaseModel):
    ...
    voice_id: Optional[str] = Field(
        default=None, description="ElevenLabs voice ID (auto-matched to avatar gender if not provided)"
    )
    ...
    caption_style: Optional[str] = Field(
        default=None, description="Caption style: classic, bold, neon, minimal, karaoke, boxed, gradient"
    )
    ...
    pip_position: str = Field(
        default="bottom-right", description="PIP position: bottom-right, bottom-left, top-right, top-left"
    )
    pip_size: float = Field(default=0.35, ge=0.2, le=0.5, description="PIP size as fraction of screen width (0.2-0.5)")
    pip_remove_background: bool = Field(
        default=True, description="Remove avatar background for seamless video blending"
    )
    # Face Swap options
    face_swap_image: Optional[str] = Field(default=None, description="URL or base64 of user's face to swap onto avatar")
    face_swap_hair_source: str = Field(
        default="user", description="Hair source: 'user' (keep user's hair) or 'target' (keep avatar's hair)"
    )
...
async def generate_video_from_prompt(request: GenerateVideoFromPromptRequest, user_id: str = "demo-user"):
    """
    ...
    print(
        f"[API] Face swap image received: {bool(face_swap_value)} (length: {len(face_swap_value) if face_swap_value else 0})",
        flush=True,
    )
    if face_swap_value:
...
async def update_video_scene(job_id: str, scene_id: str, request: UpdateSceneRequest):
...
async def add_video_scene(job_id: str, request: AddSceneRequest):
    """Add a new scene to a video project"""
    generator = get_video_generator()

    scene = await generator.add_scene(job_id, request.model_dump(), request.after_scene_id)
...
async def get_music_library(mood: Optional[str] = None, genre: Optional[str] = None):
...
async def download_generated_video(job_id: str):
    ...
    return FileResponse(
        path=str(video_path), media_type="video/mp4", filename=f"{job.project.title if job.project else job_id}.mp4"
    )
...
class GenerateScriptRequest(BaseModel):
    ...
    style: str = Field(
        default="educational", description="Script style: educational, entertaining, motivational, tutorial"
    )
...
class GenerateFromScriptRequest(BaseModel):
    ...
    pip_bg_color: Optional[str] = Field(
        default=None, description="Background color to remove (hex format like 0xRRGGBB, auto-detect if None)"
    )
    pip_bg_similarity: float = Field(
        default=0.3, ge=0.1, le=0.5, description="Color similarity threshold for background removal (0.1-0.5)"
    )
    # Face Swap options
    face_swap_image: Optional[str] = Field(default=None, description="URL or base64 of user's face to swap onto avatar")
    face_swap_hair_source: str = Field(
    ...
    # Quality/Cost optimization
    avatar_quality: str = Field(
        default="final", description="Avatar quality mode: 'draft', 'preview', 'standard', 'final'"
    )
...
async def generate_script(request: GenerateScriptRequest):
    ...
    try:
        script = await planner.generate_script_from_topic(
            topic=request.topic, duration=request.duration, style=request.style, target_audience=request.target_audience
        )
        return script
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e))
...
async def generate_video_from_script(
    request: GenerateFromScriptRequest, background_tasks: BackgroundTasks, user_id: str = "demo-user"
):
    """
    ...
    if request.enable_lipsync and request.avatar_id and request.auto_match_voice:
        ...
        voice_id = voice_service.get_voice_for_avatar(
            avatar_id=request.avatar_id, language=request.language, provider=request.voice_provider
        )
    ...
    project = await planner.script_to_video_project(
        script_data=request.script, format=request.format, voice_id=voice_id
    )
    ...
    face_swap_value = request.face_swap_image
    print(
        f"[API-Script] Face swap image received: {bool(face_swap_value)} (length: {len(face_swap_value) if face_swap_value else 0})",
        flush=True,
    )
    if face_swap_value:
    ...
    job = await generator.generate_video_from_project(project=project, request=gen_request, user_id=user_id)
...
async def analyze_scene_visual(request: AnalyzeSceneRequest):
    ...
    analysis = await router.analyze_only(description=request.description, context=request.script_context)
...
class GenerateDiagramRequest(BaseModel):
    ...
    diagram_type: Optional[str] = Field(
        default=None, description="flowchart, sequence, architecture, class, er, mindmap, state, gantt"
    )
...
async def preview_mermaid(request: PreviewMermaidRequest):
    ...
    preview = await diagram_gen.preview_mermaid(description=request.description, diagram_type=diagram_type)
...
async def get_avatar_gallery(style: Optional[str] = None, gender: Optional[str] = None, user_id: str = "demo-user"):
    """
    ...
    gallery = avatar_service.get_avatar_gallery(user_id=user_id, style=style_filter, gender=gender_filter)
...
async def generate_avatar_video(request: AvatarVideoGenerateRequest, user_id: str = "demo-user"):
    """
    ...
    if not avatar_service:
        raise HTTPException(
            status_code=503, detail="Avatar service not configured. Set DID_API_KEY environment variable."
        )
    ...
    try:
        result = await avatar_service.generate_avatar_video(request=avatar_request, user_id=user_id)
        ...
        return AvatarVideoResponse(job_id=str(uuid.uuid4()), status="failed", provider="d-id", error_message=str(e))
...
async def create_custom_avatar(request: CustomAvatarUploadRequest, user_id: str = "demo-user"):
    """
    ...
    custom_request = CustomAvatarRequest(photo_url=request.photo_url, user_id=user_id, name=request.name, style=style)
...
async def get_available_voices_for_avatars(
    language: str = "en", provider: str = "elevenlabs", gender: Optional[str] = None
):
    """
    ...
    voice_service = get_voice_service()
    voices = voice_service.get_available_voices(language=language, provider=provider, gender=gender)
...
async def get_voice_for_avatar(avatar_id: str, language: str = "en", provider: str = "elevenlabs"):
    """
    ...
    voice_service = get_voice_service()
    voice_id = voice_service.get_voice_for_avatar(avatar_id=avatar_id, language=language, provider=provider)
...
async def swap_face(request: FaceSwapRequest):
    ...
    if not face_swap.is_available():
        raise HTTPException(status_code=503, detail="Face swap service not available. Check REPLICATE_API_KEY.")
    ...
    try:
        ...
        if result_path:
            return FaceSwapResponse(success=True, image_url=result_path)
        else:
            return FaceSwapResponse(success=False, error_message="Face swap failed. Check logs for details.")

    except Exception as e:
        return FaceSwapResponse(success=False, error_message=str(e))


@app.post("/api/v1/media/avatars/create-from-faceswap", response_model=CreateFaceSwapAvatarResponse)
async def create_avatar_from_faceswap(request: CreateFaceSwapAvatarRequest, user_id: str = "demo-user"):
    """
    ...
    if not base_avatar:
        raise HTTPException(status_code=404, detail=f"Base avatar '{request.base_avatar_id}' not found")

    # Get base avatar image URL
    base_image_url = base_avatar.get("did_presenter_id") or base_avatar.get("preview_url")

    if not base_image_url:
        raise HTTPException(status_code=400, detail=f"Base avatar '{request.base_avatar_id}' has no image URL")
    ...
    try:
        ...
        if result:
            ...
            return CreateFaceSwapAvatarResponse(
                success=False, error_message="Failed to create custom avatar. Check logs for details."
            )

    except Exception as e:
        return CreateFaceSwapAvatarResponse(success=False, error_message=str(e))
...
async def upload_segment(
    ...
    await timeline.update_segment_duration(project_id, segment.id, duration, thumbnail_url)
...
async def split_segment(
    ...
    return {"message": "Segment split successfully", "first_half": first_half, "second_half": second_half}
...
async def start_render(
    ...
    if request.include_watermark and request.watermark_url:
        watermark = ImageOverlay(
            image_url=request.watermark_url, position_x=0.95, position_y=0.05, scale=0.1, opacity=0.7
        )
    ...
    return {"job_id": job_id, "project_id": project_id, "status": "pending", "message": "Render job started"}
...
async def create_preview(
    ...
    try:
        ...
        return {"preview_url": preview_url, "start_time": start_time, "duration": duration}
    except Exception as e:
...
async def serve_editor_thumbnail(filename: str):
    ...
    return FileResponse(path=str(thumb_path), media_type="image/jpeg", filename=filename)
...
async def upload_voice_sample(
    ...
    sample, message = await manager.add_sample(profile_id, user_id, content, file.filename)
...
class HybridTTSRequest(BaseModel):
    ...
    provider: Optional[str] = Field(
        default=None, description="Force specific provider: kokoro, chatterbox, elevenlabs, openai"
    )
...
async def get_tts_providers():
    ...
    try:
        ...
        return {"error": str(e), "available_providers": [], "fallback_mode": True}
...
async def generate_hybrid_tts(request: HybridTTSRequest):
    ...
    try:
        ...
        gender = (
            ...
            if request.voice_gender in ["male", "female", "neutral"]
            else VoiceGender.NEUTRAL
        )
        quality = (
            TTSQuality(request.quality) if request.quality in ["draft", "standard", "premium"] else TTSQuality.STANDARD
        )
