...
class PredefinedAvatar(BaseModel):
    ...
    did_presenter_id: str = Field(description="D-ID presenter/source ID for Talks API (face animation only)")
    did_clip_presenter_id: Optional[str] = Field(
        default=None, description="D-ID presenter ID for Clips API (full body movement)"
    )
    heygen_avatar_id: Optional[str] = Field(default=None, description="HeyGen avatar ID for fallback")
    style: AvatarStyle = Field(description="Avatar visual style")
    gender: AvatarGender = Field(description="Avatar gender")
    description: Optional[str] = Field(default=None, description="Brief description of the avatar")
    supported_languages: List[str] = Field(
        default_factory=lambda: ["en"], description="Languages this avatar supports well"
    )
    is_premium: bool = Field(default=False, description="Whether this is a premium avatar")
...
class AvatarVideoRequest(BaseModel):
    ...
    script_text: Optional[str] = Field(default=None, description="Text script for TTS (if no voiceover provided)")
    voice_id: Optional[str] = Field(default=None, description="Voice ID for TTS generation")
    voiceover_url: Optional[str] = Field(default=None, description="Pre-generated voiceover audio URL")
    output_format: str = Field(default="9:16", description="Output aspect ratio (9:16, 16:9, 1:1)")
    background_color: Optional[str] = Field(default=None, description="Background color (hex) or 'transparent'")
    expression: Optional[str] = Field(default="neutral", description="Avatar expression: neutral, happy, serious")
    driver_type: Optional[str] = Field(default="microsoft", description="D-ID driver type for lip-sync")
    quality: str = Field(
        default="final",
...
class AvatarVideoResult(BaseModel):
    ...
    status: str = Field(default="completed", description="Generation status: pending, processing, completed, failed")
    ...
    cost_estimate: Optional[float] = Field(default=None, description="Estimated cost in USD for this generation")
    quality: Optional[str] = Field(default=None, description="Quality mode used: draft, preview, or final")
...
class CustomAvatarRequest(BaseModel):
    ...
    style: AvatarStyle = Field(default=AvatarStyle.PROFESSIONAL, description="Desired avatar style")
...
class CustomAvatarResult(BaseModel):
    ...
    provider_source_id: str = Field(description="D-ID source ID for the custom photo")
    processing_status: str = Field(default="ready", description="Processing status: processing, ready, failed")


class AvatarGalleryResponse(BaseModel):
    """Response containing the avatar gallery."""

    avatars: List[PredefinedAvatar] = Field(description="List of available avatars")
    total_count: int = Field(description="Total number of avatars")
    styles: List[str] = Field(
        default_factory=lambda: [s.value for s in AvatarStyle], description="Available styles for filtering"
    )
    genders: List[str] = Field(
        default_factory=lambda: [g.value for g in AvatarGender], description="Available genders for filtering"
    )
