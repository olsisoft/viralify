...
class VisualAnalysis(BaseModel):
    ...
    visual_type: VisualType = Field(description="Recommended visual type for this scene")
    ...
    diagram_type: Optional[DiagramType] = Field(
        default=None, description="Specific diagram type if visual_type is DIAGRAM"
    )
    chart_type: Optional[ChartType] = Field(default=None, description="Specific chart type if visual_type is CHART")
    requires_avatar: bool = Field(default=False, description="Whether the scene would benefit from an avatar presenter")
    extracted_elements: List[str] = Field(
        default_factory=list, description="Key elements extracted from the description for diagram generation"
    )
    extracted_relationships: List[Dict[str, str]] = Field(
        default_factory=list, description="Relationships between elements (from, to, label)"
    )
    mermaid_possible: bool = Field(default=False, description="Whether Mermaid.js can accurately represent this visual")
    domain: Optional[str] = Field(
        default=None, description="Detected domain: software, architecture, business, data, etc."
    )
    reasoning: str = Field(default="", description="GPT-4's reasoning for the recommendation")
    suggested_prompt: Optional[str] = Field(
        default=None, description="Enhanced prompt for DALL-E if using AI generation"
    )
...
class DiagramResult(BaseModel):
    ...
    mermaid_code: Optional[str] = Field(default=None, description="Mermaid code if generated with Mermaid.js")
    ...
    fallback_used: bool = Field(default=False, description="Whether DALL-E fallback was used due to Mermaid failure")
...
class VisualGenerationRequest(BaseModel):
    ...
    script_context: Optional[str] = Field(
        default=None, description="Surrounding script context for better understanding"
    )
    preferred_type: Optional[VisualType] = Field(
        default=None, description="User-preferred visual type (overrides auto-detection)"
    )
    ...
    avatar_id: Optional[str] = Field(default=None, description="Avatar ID if using avatar visual")
    voiceover_url: Optional[str] = Field(default=None, description="Voiceover URL for avatar lip-sync")
