...
class CurriculumEnforcerService:
    ...
    async def enforce(self, request: EnforcementRequest) -> EnforcementResult:
        """
        ...
        try:
            ...
            if not template:
                return EnforcementResult(
                    ...
                    success=False,
                    original_validation=ValidationResult(is_valid=False, score=0.0, violations=[], suggestions=[]),
                    template_used="none",
            ...
            original_validation = await self.validator.validate(content=request.content, template=lesson_template)
            ...
            final_validation = await self.validator.validate(content=restructured["content"], template=lesson_template)
    ...
    async def _restructure_content(
        ...
        for missing_phase in validation.missing_required_phases:
            ...
            phase_config = next((p for p in template.phases if p.phase == missing_phase), None)
            ...
            if phase_config:
                ...
                if self.client:
                    ...
                    generated_slide = self._create_placeholder_slide(phase=missing_phase, config=phase_config)

                # Insert at correct position
                insert_position = self._find_insert_position(
                    phase_config=phase_config, detected_phases=validation.detected_phases, total_slides=len(new_slides)
                )
        ...
        if not preserve_content and not template.allow_phase_reordering:
            new_slides, reorder_changes = self._reorder_slides(
                slides=new_slides, template=template, detected_phases=validation.detected_phases
            )
        ...
        return {"content": restructured_content, "changes": changes}

    async def _generate_phase_content(
        self, phase: LessonPhase, config: PhaseConfig, lesson_context: str, existing_content: List[Dict]
    ) -> Dict[str, Any]:
        """
        ...
        content_summary = "\n".join(
            [f"- {s.get('title', 'Slide')}: {str(s.get('content', ''))[:100]}..." for s in existing_content[:5]]
        )
...
Generate the {phase.value} phase content."""
    ...
    def _create_placeholder_slide(self, phase: LessonPhase, config: PhaseConfig) -> Dict[str, Any]:
        """
    ...
    def _find_insert_position(self, phase_config: PhaseConfig, detected_phases: List[Dict], total_slides: int) -> int:
        """
    ...
    def _reorder_slides(
        self, slides: List[Dict], template: LessonTemplate, detected_phases: List[Dict]
    ...
    async def validate_only(
        ...
        if not template:
            return ValidationResult(
                ...
                violations=[
                    PhaseViolation(
                        phase=LessonPhase.HOOK, violation_type="missing", message="Template not found", severity="error"
                    )
                ],
                suggestions=["Provide a valid template"],
            )

        return await self.validator.validate(content=content, template=template.default_lesson_template)
    ...
    def get_template_phases(self, context_type: ContextType) -> List[Dict[str, Any]]:
        """
    ...
    def register_custom_template(self, template: CurriculumTemplate) -> None:
        """
...
# Factory function
def create_curriculum_enforcer(openai_api_key: Optional[str] = None) -> CurriculumEnforcerService:
