...
async def enforce_lesson_structure(
    lesson_data: Dict[str, Any], context_type: ContextType = ContextType.EDUCATION
) -> Dict[str, Any]:
    """
    ...
    request = EnforcementRequest(content=content, context_type=context_type, auto_fix=True, preserve_content=True)
    ...
    if result.restructured_content:
        return {
            ...
            "validated": True,
            "validation_score": result.final_validation.score
            if result.final_validation
            else result.original_validation.score,
        }
...
async def validate_course_structure(
    course_outline: Dict[str, Any], context_type: ContextType = ContextType.EDUCATION
) -> Dict[str, Any]:
    """
    ...
    for section in course_outline.get("sections", []):
        ...
        validated_sections.append({**section, "lectures": validated_lessons})
...
async def main():
    ...
    print("=" * 60)
    validated_edu = await validate_course_structure(sample_course, ContextType.EDUCATION)

    # Validate with ENTERPRISE context
    print("\n" + "=" * 60)
    print("VALIDATING WITH ENTERPRISE CONTEXT")
    print("=" * 60)
    validated_ent = await validate_course_structure(sample_course, ContextType.ENTERPRISE)
