...
models_module = import_module_from_file("weave_graph_models", os.path.join(weave_graph_dir, "models.py"))
...
edge_weight_module = import_module_from_file(
    "edge_weight_calculator", os.path.join(weave_graph_dir, "edge_weight_calculator.py")
)
...
class TestGraphBuilderConfigIntegration:
    ...
    def test_edge_weight_config_from_builder_config(self):
        ...
        config = EdgeWeightConfig(
            cooccurrence_weight=0.5, hierarchy_weight=0.3, embedding_weight=0.2, min_edge_weight=0.2, window_size=2
        )
...
class TestEdgeWeightCalculatorWithConcepts:
    ...
    def calculator(self):
        ...
        config = EdgeWeightConfig(
            cooccurrence_weight=0.4, hierarchy_weight=0.3, embedding_weight=0.3, min_edge_weight=0.1
        )
    ...
    def test_calculate_weight_between_concepts(self, calculator, tech_concepts):
        ...
        # Setup
        chunks = ["Kafka consumer reads events", "Producer writes to Kafka"]
        calculator.train_cooccurrence(chunks)
...
class TestConceptEdgeCreation:
    ...
    def test_create_edge_from_weight(self):
        ...
        edge = ConceptEdge(
            source_id=source_id, target_id=target_id, relation_type=rel_type, weight=weight, bidirectional=True
        )
...
class TestMultiFactorWeighting:
    ...
    def test_all_factors_contribute(self):
        ...
        config = EdgeWeightConfig(cooccurrence_weight=0.4, hierarchy_weight=0.3, embedding_weight=0.3)
        calc = EdgeWeightCalculator(config)

        # Train cooccurrence
        calc.train_cooccurrence(["Kafka and consumer work together frequently.", "Consumer reads from Kafka broker."])

        # Set embeddings (similar vectors)
        calc.set_embeddings({"kafka": [1.0, 0.0, 0.0], "consumer": [0.95, 0.05, 0.0]})
    ...
    def test_weight_normalization(self):
        ...
        calc.train_cooccurrence(["kafka consumer kafka consumer kafka consumer"])
        calc.set_embeddings(
            {
                "kafka": [1.0, 0.0, 0.0],
