...
class CodeExecutorService:
    ...
    LANGUAGE_CONFIGS = {
        "python": {"extension": ".py", "command": ["python3", "-u"], "timeout": 30},
        "javascript": {"extension": ".js", "command": ["node"], "timeout": 30},
        "bash": {"extension": ".sh", "command": ["bash"], "timeout": 30},
        "shell": {"extension": ".sh", "command": ["sh"], "timeout": 30},
    }
    ...
    async def execute(self, code: str, language: str, timeout: Optional[int] = None) -> ExecutionResult:
        """
    ...
    async def _execute_local(self, code: str, language: str, timeout: int) -> ExecutionResult:
        ...
        try:
            ...
            try:
                ...
                process = await asyncio.create_subprocess_exec(
                    *cmd, stdout=asyncio.subprocess.PIPE, stderr=asyncio.subprocess.PIPE, cwd=tempfile.gettempdir()
                )
            ...
            return ExecutionResult(success=False, error=str(e), execution_time=time.time() - start_time)

    async def _execute_replicate(self, code: str, language: str, timeout: int) -> ExecutionResult:
        ...
        if not model:
            return ExecutionResult(success=False, error=f"No Replicate model for language: {language}")
        ...
        try:
            async with httpx.AsyncClient(timeout=timeout + 10) as client:
                ...
                response = await client.post(
                    ...
                    headers={"Authorization": f"Token {self.replicate_api_key}", "Content-Type": "application/json"},
                    json={"version": model, "input": {"code": code, "timeout": timeout}},
                )

                if response.status_code != 201:
                    return ExecutionResult(
                        success=False, error=f"Replicate API error: {response.status_code} - {response.text}"
                    )
                ...
                return ExecutionResult(
                    success=False, error="Replicate execution timeout", execution_time=time.time() - start_time
                )
