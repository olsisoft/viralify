...
class VoiceoverEnforcer:
    ...
    def validate_script(self, script_data: dict, target_duration: int) -> list[VoiceoverValidation]:
        """
    ...
    async def expand_short_voiceovers(
        self, script_data: dict, validations: list[VoiceoverValidation], content_language: str = "fr"
        ...
        if not short_slides:
            print("[ENFORCER] All voiceovers valid", flush=True)
        ...
        async def expand_one(validation: VoiceoverValidation) -> tuple[int, str]:
            async with semaphore:
                ...
                expanded = await self._expand_voiceover(slide, validation.required_words, content_language)
        ...
        for result in results:
            ...
            if expanded_voiceover and len(expanded_voiceover.split()) > validations[idx].word_count:
                ...
                expanded_count += 1
                print(
                    f"[ENFORCER] Slide {idx + 1} ({slides[idx].get('type', 'content')}): {old_count} -> {new_count} words",
                    flush=True,
                )
        ...
        final_words = sum(
            len(re.sub(r"\[SYNC:[\w_]+\]", "", s.get("voiceover_text", "") or "").split()) for s in slides
        )
        target_words = sum(v.required_words for v in validations)
        ...
        print(
            f"[ENFORCER] Expansion complete: {original_words} -> {final_words} words ({result.duration_ratio:.0%})",
            flush=True,
        )

        return script_data, result

    async def _expand_voiceover(self, slide: dict, target_words: int, language: str) -> str:
        """
...
OUTPUT: Return ONLY the expanded voiceover text, nothing else. No quotes, no explanations."""
        ...
        try:
            ...
            if expanded_words > current_words:
                ...
                print(
                    f"[ENFORCER] Expansion didn't increase words ({current_words} -> {expanded_words}), keeping original",
                    flush=True,
                )
                return current_voiceover
...
async def enforce_voiceover_duration(
    script_data: dict, target_duration: int, content_language: str = "fr", client: Optional[AsyncOpenAI] = None
) -> tuple[dict, EnforcementResult]:
    """
    ...
    print(
        f"[ENFORCER] Validation: {total_words}/{target_words} words, {invalid_count}/{len(validations)} slides need expansion",
        flush=True,
    )

    # Expand if needed
    if invalid_count > 0:
        return await enforcer.expand_short_voiceovers(script_data, validations, content_language)
    else:
        return script_data, EnforcementResult(
