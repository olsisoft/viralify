...
class Slide(BaseModel):
    ...
    script_segments: List[ScriptSegment] = Field(
        default_factory=list, description="Segmented script: intro, explanation, example, summary"
    )
    ...
    diagram_type: Optional[str] = Field(
        None, description="Type of diagram: flowchart, architecture, process, comparison, hierarchy"
    )
    ...
    bloom_level: Optional[str] = Field(
        None, description="Bloom's taxonomy level: remember, understand, apply, analyze, evaluate, create"
    )
...
class PresentationScript(BaseModel):
    ...
    target_career: Optional[str] = Field(
        default=None, description="Target career for diagram focus (e.g., 'data_engineer', 'cloud_architect')"
    )
...
class RAGImageReference(BaseModel):
    ...
    detected_type: str = Field(
        default="diagram", description="Image type: diagram, chart, architecture, flowchart, screenshot, photo"
    )
...
class GeneratePresentationRequest(BaseModel):
    ...
    content_language: str = Field(
        default="en", description="Language for content (voiceover, titles, text). Examples: 'en', 'fr', 'es', 'de'"
    )
    ...
    typing_speed: TypingSpeed = Field(
        default=TypingSpeed.NATURAL, description="Typing animation speed: slow, natural, moderate, fast"
    )
    code_display_mode: CodeDisplayMode = Field(
        default=CodeDisplayMode.REVEAL,
    ...
    target_career: Optional[str] = Field(
        default=None,
        description="Target career for diagram focus (e.g., 'data_engineer', 'cloud_architect'). See TechCareer enum for full list.",
    )
    title_style: Optional[TitleStyle] = Field(
        default=TitleStyle.ENGAGING,
    ...
    document_ids: List[str] = Field(
        default_factory=list, description="IDs of uploaded documents to use as source material (from course-generator)"
    )
    rag_context: Optional[str] = Field(
        None, description="Pre-fetched RAG context (set by server after querying documents)"
    )
    source_topics: Optional[List[str]] = Field(
        None, description="Pre-extracted topics from RAG context (avoids re-extraction per lesson)"
    )
    use_documents_for_diagrams: bool = Field(
        default=True, description="Extract and use diagrams/schemas from source documents"
    )
    # RAG images extracted from documents (for diagram slides - use real images instead of LLM generation)
    rag_images: List[RAGImageReference] = Field(
        default_factory=list, description="Images extracted from RAG documents to use in diagram slides"
    )
    # Practical focus level - affects slide type distribution
    practical_focus: Optional[str] = Field(
        default=None,
...
class PresentationJob(BaseModel):
    ...
    code_execution_results: Dict[str, Any] = Field(
        default_factory=dict, description="Code execution results by slide ID"
    )
