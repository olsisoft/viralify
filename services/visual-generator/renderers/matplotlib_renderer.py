...
class MatplotlibRenderer:
    ...
    def __init__(self, openai_api_key: Optional[str] = None, output_dir: str = "/tmp/viralify/charts"):
...
5. Include 1-3 data series max"""
    ...
    def render(
        ...
        try:
            ...
            return DiagramResult(
                ...
                metadata={"chart_title": chart.title, "series_count": len(chart.data_series)},
            )
    ...
    def _render_line_chart(self, ax: plt.Axes, chart: MatplotlibChart, x_values: List, palette: List[str]):
        """Render a line chart."""
    ...
    def _render_bar_chart(self, ax: plt.Axes, chart: MatplotlibChart, x_values: List, palette: List[str]):
        """Render a bar chart."""
        ...
        for i, series in enumerate(chart.data_series):
            ...
            ax.bar(x + offset, series.values[: len(x_values)], bar_width, label=series.name, color=color, alpha=0.9)

        ax.set_xticks(x)
        ax.set_xticklabels(x_values, rotation=45, ha="right")

    def _render_pie_chart(self, ax: plt.Axes, chart: MatplotlibChart, palette: List[str]):
        """Render a pie chart."""
        if chart.data_series:
    ...
    def _render_scatter_plot(self, ax: plt.Axes, chart: MatplotlibChart, x_values: List, palette: List[str]):
        """Render a scatter plot."""
        for i, series in enumerate(chart.data_series):
            color = series.color or palette[i % len(palette)]
            ax.scatter(x_values[: len(series.values)], series.values, label=series.name, color=color, s=80, alpha=0.7)

    def _render_histogram(self, ax: plt.Axes, chart: MatplotlibChart, palette: List[str]):
        """Render a histogram."""
        for i, series in enumerate(chart.data_series):
            color = series.color or palette[i % len(palette)]
            ax.hist(series.values, bins=10, label=series.name, color=color, alpha=0.7, edgecolor="white")

    def _render_heatmap(self, ax: plt.Axes, chart: MatplotlibChart, palette: List[str]):
        """Render a heatmap."""
    ...
    def _render_box_plot(self, ax: plt.Axes, chart: MatplotlibChart, palette: List[str]):
        """Render a box plot."""
    ...
    async def generate_and_render(
        ...
        chart = await self.generate_from_description(
            description=description, chart_type=chart_type, style=style, context=context, language=language
        )

        # Render to image
        return self.render(chart=chart, style=style, format=format, width=width, height=height)
